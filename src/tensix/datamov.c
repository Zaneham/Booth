                       ...
                       const char *path)
{
    ...
    dm_write(fp,
        "/* Generated by Booth — Tensix reader kernel.\n"
                       ...
                       const char *path)
{
    ...
    dm_write(fp,
        "/* Generated by Booth — Tensix writer kernel.\n"
                          ...
                          const char *compute_path, const char *writer_path)
{
    ...
    dm_write(fp,
        "/* Generated by Booth — Tensix host program.\n"
        " * Three kernels, many cores, one dream.\n"
        " * Element count and buffer files come from argv: the grid is a launch-time\n"
        " * quantity and the compiler is in no position to guess it. */\n"
        "\n"
        ...
        "\n"
        "static std::vector<uint32_t> ld_buf(const char *p, uint32_t nb) {\n"
        "    std::vector<uint32_t> v(nb / sizeof(uint32_t), 0u);\n"
        ...
        "}\n"
        "\n"
        "static void st_buf(const char *p, const std::vector<uint32_t> &v) {\n"
    ...
    guard = TT_MAX_DMOV_BUFS;
    for (uint32_t i = 0; i < dmov->num_bufs && guard > 0; i++, guard--) {
        if (dmov->bufs[i].is_output) nout++; else nin++;
    }

    dm_write(fp,
        "    if (argc != %u) {\n"
        "        std::fprintf(stderr,\n"
        "                     \"usage: %%s <elements>%s%s\\n\", argv[0]);\n"
        "        return 2;\n"
        "    }\n"
        "    uint32_t total_elements =\n"
        "        (uint32_t)std::strtoul(argv[1], nullptr, 10);\n"
        "    if (total_elements == 0u) {\n"
        "        std::fprintf(stderr, \"%%s: element count must be non-zero\\n\",\n"
        "                     argv[0]);\n"
        "        return 2;\n"
        "    }\n"
        "\n"
        ...
        "    uint32_t num_tiles =\n"
        "        (total_elements + elements_per_tile - 1) / elements_per_tile;\n"
    ...
    dm_write(fp, "    /* Upload every input named on the command line */\n");
    {
        uint32_t a = 2u;
        guard = TT_MAX_DMOV_BUFS;
        for (uint32_t i = 0; i < dmov->num_bufs && guard > 0; i++, guard--) {
            ...
            if (buf->is_output) continue;
            dm_write(fp,
                "    std::vector<uint32_t> host_in%u =\n"
                "        ld_buf(argv[%u], dram_buf_size);\n"
                "    EnqueueWriteBuffer(cq, buf_in%u, host_in%u, false);\n",
                buf->cb_index, a, buf->cb_index, buf->cb_index);
            a++;
        }
        ...
        dm_write(fp,
            ...
            "    Finish(cq);\n"
            "\n"
            "    /* Download every output named on the command line */\n");
        ...
        for (uint32_t i = 0; i < dmov->num_bufs && guard > 0; i++, guard--) {
            ...
            on = buf->cb_index - 16;
            dm_write(fp,
                "    std::vector<uint32_t> host_out%u(dram_buf_size / sizeof(uint32_t), 0u);\n"
                "    EnqueueReadBuffer(cq, buf_out%u, host_out%u, true);\n"
                "    st_buf(argv[%u], host_out%u);\n",
                on, on, on, a, on);
            a++;
        }
    }
                             ...
                             const char *stem)
{
    ...
    printf("  NOTE: compute load/store still bind to Dst row 0; the unpack/pack\n"
           "        <-> CB-buffer binding is not emitted, so the compute core does\n"
           "        not yet consume the delivered tiles.\n");
    return BC_OK;
}
