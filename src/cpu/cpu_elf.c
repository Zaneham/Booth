/* cpu_elf.c -- ELF64 relocatable object, kernel exported as a
 * global function symbol so the host can dlopen+dlsym it. Four
 * sections: .text, .shstrtab, .symtab, .strtab, plus a SHT_NULL
 * at index 0. Byte-at-a-time, no library. */
...
int cpu_elf(const cpu_mod_t *X, const char *path)
{
    ...
    uint64_t eh=64, text=eh, ss=text+X->codelen, sy=ss+(uint32_t)sh, st2=sy+48, shoff=st2+(uint32_t)st;
    p32(f,0x464c457f);fputc(2,f);fputc(1,f);fputc(1,f);fputc(0,f);for(int i=0;i<8;i++)fputc(0,f);
    p16(f,1);p16(f,62);p32(f,1);p64(f,0);p64(f,0);p64(f,shoff);p32(f,0);p16(f,64);p16(f,0);p16(f,0);p16(f,64);p16(f,5);p16(f,2);
    fwrite(X->code,1,X->codelen,f); fwrite(shstr,1,(size_t)sh,f);
    p32(f,0);p32(f,0);p64(f,0);p64(f,0); p32(f,s_k);fputc(0x12,f);fputc(0,f);p16(f,1);p64(f,0);p64(f,X->codelen); /* sym1: kernel */
    fwrite(strt,1,(size_t)st,f);
    p32(f,0);p32(f,0);p64(f,0);p64(f,0);p64(f,0);p64(f,0);p32(f,0);p32(f,0);p64(f,0);p64(f,0); /* sh0 null */
    p32(f,o_text);p32(f,1);p64(f,6);p64(f,0);p64(f,text);p64(f,X->codelen);p32(f,0);p32(f,0);p64(f,16);p64(f,0); /* .text */
